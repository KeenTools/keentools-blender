...
class FB_OT_ReducePins(Operator):
    bl_idname = Config.fb_reduce_pins_idname
    bl_label = "Reduce pins"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}
    bl_description = "Reduce all pins"
    ...
    def execute(self, context):
        ...
        FBLoader.fb_save(headnum, camnum)
        manipulate.push_head_in_undo_history(
            settings.get_head(headnum), 'Before Reduce pins')
        ...
        manipulate.push_head_in_undo_history(
            settings.get_head(headnum), 'After Reduce pins')
