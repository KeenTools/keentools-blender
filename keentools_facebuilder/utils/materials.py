...
def bake_tex2(headnum, tex_name):
    logger = logging.getLogger(__name__)
    settings = get_main_settings()
    head = settings.get_head(headnum)
    # Add UV
    ...
    for i, cam in enumerate(head.cameras):
        ...
        if cam.use_in_tex_baking and cam.cam_image and cam.has_pins():
            pix = cam.cam_image.pixels[:]
            img = np.rot90(np.asarray(pix).reshape((h, w, 4)), cam.orientation)
            imgs.append(img)
            keyframes.append(cam.get_keyframe())
            camnums.append(i)
    wm.progress_end()
    ...
    if len(keyframes) > 0:
        import pykeentools
        tb = pykeentools.TextureBuilder()
        # build_texture(self: pykeentools.TextureBuilder, geos: List[
        #     pykeentools.Geo], imgs: List[img], model_views: List[
        #     mat4], projections: List[mat4]) -> img

        offset = np.array([[1., 0., 0., (h - w)/2],
                        [0., 1., 0., (w - h)/2],
                        [0., 0., 1., 0.],
                        [0., 0., 0., 1.]])
        geos = [geo for _ in keyframes]
        model_views = [head.cameras[x].get_model_mat() for x in camnums]
        # projections = [fb.projection_mat() for _ in camnums]
        projections = []
        for n in camnums:
            if head.cameras[n].orientation in (1, 3):
                projections.append(offset @ fb.projection_mat())
            else:
                projections.append(fb.projection_mat())

        print('P:', projections)

        tb.set_output_texture_size((tw, th))
        tb.set_face_angles_affection(tfaa)
        tb.set_uv_expand_percents(tuep)
        print(tuep)
        tb.set_back_face_culling(tbfc)
        tb.set_equalize_brightness(teb)
        tb.set_equalize_colour(tec)
        ...
        tex = bpy.data.images.new(
                ...
                alpha=True, float_buffer=False)
        # Store Baked Texture into blender
        print('t:', texture.shape)
        buffer = texture.ravel()
        print(buffer.shape)
        tex.pixels[:] = buffer
        # Pack image to store in blend-file
        tex.pack()
