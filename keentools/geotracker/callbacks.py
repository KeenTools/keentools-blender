...
_gt_old_focal_length_mm: float = 50.0
_gt_camobj_lens_watcher_owner = _Owner()
_gt_movie_clip_color_space_watcher_owner = _Owner()
_gt_movie_clip_frame_start_watcher_owner = _Owner()
_gt_mask_2d_frame_start_watcher_owner = _Owner()


def _set_old_focal_length_mm(value: float):
    global _gt_old_focal_length_mm
    _gt_old_focal_length_mm = value


def unsubscribe_watcher(owner: object) -> None:
    _log.green(f'gt unsubscribe_watcher start: {owner}')
    bpy_msgbus_clear_by_owner(owner)
    _log.output(f'gt unsubscribe_watcher end >>>')


def subscribe_camera_lens_watcher(camobj: Optional[Object]) -> None:
    _log.green('gt subscribe_camera_lens_watcher start')
    unsubscribe_watcher(_gt_camobj_lens_watcher_owner)
    if not camobj or not camobj.data:
        _log.red('gt subscribe_camera_lens_watcher: no camera object')
        return
    ...
    bpy_msgbus_subscribe_rna(key=subscribe_to,
                             owner=_gt_camobj_lens_watcher_owner,
                             args=(),
                             notify=lens_change_callback)
    _log.output('gt subscribe_camera_lens_watcher end >>>')


def subscribe_movie_clip_color_space_watcher(geotracker: Any) -> None:
    _log.green('subscribe_movie_clip_color_space_watcher start')
    unsubscribe_watcher(_gt_movie_clip_color_space_watcher_owner)
    ...
    bpy_msgbus_subscribe_rna(key=subscribe_to,
                             owner=_gt_movie_clip_color_space_watcher_owner,
...
def color_space_change_callback(old_name: str) -> None:
    _log.green('color_space_change_callback start')
...
def subscribe_movie_clip_frame_start_watcher(geotracker: Any) -> None:
    ...
    unsubscribe_watcher(_gt_movie_clip_frame_start_watcher_owner)
    if not geotracker or not geotracker.movie_clip:
    ...
    bpy_msgbus_subscribe_rna(key=subscribe_to,
                             owner=_gt_movie_clip_frame_start_watcher_owner,
                             args=(),
...
def subscribe_mask_2d_frame_start_watcher(geotracker: Any) -> None:
    ...
    unsubscribe_watcher(_gt_mask_2d_frame_start_watcher_owner)
    ...
    bpy_msgbus_subscribe_rna(key=subscribe_to,
                             owner=_gt_mask_2d_frame_start_watcher_owner,
                             args=(),
...
def recalculate_focal(use_current_frame: bool = True) -> bool:
    _log.yellow('gt recalculate_focal start')
    settings = gt_settings()
    geotracker = settings.get_current_geotracker_item()
    ...
    _log.magenta('gt recalculate_model_for_new_focal_length')
    ...
    _log.output('gt recalculate_focal end >>>')
    return True


def lens_change_callback() -> None:
    _log.magenta('gt lens_change_callback start')
    settings = gt_settings()
    if settings.ui_write_mode:
        _log.green('gt lens_change_callback ui_write_mode >>>')
        return
    ...
    if settings.pinmode:
        loader.update_viewport_shaders(wireframe_data=True,
                                       ...
                                       timeline=True)
    _log.output('gt lens_change_callback end >>>')


def update_camobj(geotracker, context: Any) -> None:
    _log.green(f'update_camobj: {geotracker.camobj} start')
    settings = gt_settings()
...
def update_geomobj(geotracker, context: Any) -> None:
    _log.green(f'update_geomobj: {geotracker.geomobj} start')
    settings = gt_settings()
...
def update_movieclip(geotracker, context: Any) -> None:
    _log.green('update_movieclip start')
    if not geotracker:
        return
...
def update_precalc_path(geotracker, context: Any) -> None:
    _log.green('update_precalc_path start')
    settings = gt_settings()
    if settings.ui_write_mode:
        return
    ending = '.precalc'
...
def update_wireframe(settings, context: Any) -> None:
    _log.green('update_wireframe start')
...
def update_mask_2d_color(settings, context: Any) -> None:
    _log.green('update_mask_2d_color start')
...
def update_wireframe_backface_culling(settings, context: Any) -> None:
    _log.green('update_wireframe_backface_culling start')
    if settings.ui_write_mode:
        return
    gt = GTLoader.kt_geotracker()
...
def update_pin_size(settings, context: Any) -> None:
    _log.green('update_pin_size start')
...
def update_focal_length_mode(geotracker, context: Any) -> None:
    _log.green(f'gt update_focal_length_mode: {geotracker.focal_length_mode} start')
    if geotracker.focal_length_mode == 'STATIC_FOCAL_LENGTH':
        geotracker.static_focal_length = focal_mm_to_px(
            camera_focal_length(geotracker.camobj),
            *bpy_render_frame(), camera_sensor_width(geotracker.camobj))
    _log.output('gt update_focal_length_mode end >>>')


def update_lens_mode(geotracker, context: Any=None) -> None:
    _log.green(f'gt update_lens_mode: {geotracker.lens_mode} start')
    settings = gt_settings()
    if settings.ui_write_mode:
        return
    ...
    _log.output('gt update_lens_mode end >>>')


def update_track_focal_length(geotracker, context: Any) -> None:
    _log.green('gt update_track_focal_length start')
    settings = gt_settings()
    if settings.ui_write_mode:
        return
    loader = settings.loader()
    ...
    _log.output('gt update_track_focal_length end >>>')


def update_mask_3d(geotracker, context: Any) -> None:
    _log.green('update_mask_3d start')
    settings = gt_settings()
...
def update_mask_2d(geotracker, context: Any) -> None:
    _log.green('update_mask_2d start')
    settings = gt_settings()
    settings.reload_current_geotracker()
...
def update_mask_source(geotracker, context: Any) -> None:
    _log.green('update_mask_source start')
...
def update_spring_pins_back(geotracker, context: Any) -> None:
    _log.green('update_spring_pins_back start')
...
def update_smoothing(geotracker, context: Any) -> None:
    ...
    settings = gt_settings()
    if settings.ui_write_mode:
        _log.output('update_smoothing ui_write_mode end >>>')
        return
    loader = settings.loader()
    gt = loader.kt_geotracker()
