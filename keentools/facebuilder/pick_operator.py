...
def _align_face_on_frames(
    ...
    try:
        ...
        _log.error(f'UNKNOWN EXCEPTION align_face_with_mbba '
    ...
    if result_flag:
        ...
        _log.output(f'align_face_with_mbba failed')
...
class FB_OT_AlignAllMbba(Operator):
    bl_idname = FBConfig.fb_align_all_mbba_idname
    bl_label = buttons[bl_idname].label
    bl_description = buttons[bl_idname].description
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def _action(self, context: Any, event: Any, invoked: bool=True) -> Set:
        _log.output('AlignAllMbba action call')
    ...
    def invoke(self, context: Any, event: Any) -> Set:
        _log.output('AlignAllMbba invoke call')
        settings = get_fb_settings()
        if not settings.pinmode:
            message = 'Not in pinmode call'
            self.report({'ERROR'}, message)
    ...
    def execute(self, context: Any) -> Set:  # Used only for integration testing
        _log.output('AlignAllMbba execute call')
        return self._action(context, event=None, invoked=False)
